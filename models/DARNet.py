...
class DARNet_MB(nn.Module):
    def __init__(self, chn):
        super().__init__()

        self.DAR1 = DARNet_core(chn)
        self.DAR2 = DARNet_core(chn)
        self.DAR3 = DARNet_core(chn)
        self.DAR4 = DARNet_core(chn)
        self.DAR5 = DARNet_core(chn)
    ...
    def forward(self, x):
        ...
        x_fused = torch.cat([x1, x2, x3, x4, x5], dim=-1)
        out = self.out(x_fused)
        return out